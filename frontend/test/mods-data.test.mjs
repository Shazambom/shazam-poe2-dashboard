...
test('variantName names a pool after its class and attribute tags, else after its bases', () => {
  ...
  assert.equal(variantName('Wands', ['default', 'wand'], ['Attuned Wand', 'Siphoning Wand', 'Acrid Wand', 'Volatile Wand'], true), 'Wands · Attuned, Siphoning, Acrid +1')
})
...
test('pools: one per distinct spawn-tag set within an equipment class, named as poe2db\'s index', () => {
  ...
  for (const cls of ['Jewels', 'Life Flasks', 'Charms', 'Waystones', 'Tablets', 'Relics']) assert.equal(byClass.has(cls), false, `${cls} is phase 2`)
})
...
test('mods: item-domain prefix and suffix families with tiers, no markup, # in the family text', () => {
  ...
  for (const f of families) {
    assert.ok(['prefix', 'suffix', 'corrupted'].includes(f.affix), f.id)
    assert.ok(['item', 'desecrated'].includes(f.domain), f.id)
    if (f.domain === 'desecrated') assert.ok(f.tiers.some(t => t.weights.some(([tag, w]) => w > 0 && ['ulaman_mod', 'amanamu_mod', 'kurgal_mod'].includes(tag))), `${f.id}: a desecrated family keys on a bone`)
