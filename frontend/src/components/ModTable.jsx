...
function ModTable({ title, rows, total, expanded, onToggle, ilvl, floor, empty }) {
  const idOf = (e) => e.target.closest('.mods-fam')?.dataset.id
  const onClick = (e) => { const id = idOf(e); if (id) onToggle(id) }
  const onKeyDown = (e) => {
    const id = idOf(e)
    if (!id) return
  ...
  return (
    <section className="mods-table rx-surface">
      ...
      <div className="mods-rows" onClick={onClick} onKeyDown={onKeyDown}>
        {rows.map(r => <ModFamily key={r.family.id} row={r} open={expanded.has(r.family.id)} ilvl={ilvl} floor={floor} perTier={total ? 1 / total : null} />)}
