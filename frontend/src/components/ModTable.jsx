...
function ModTable({ title, affix, rows, total, expanded, onToggle, onTrade, onStash, ilvl, floor, empty }) {
  ...
  return (
    <section className="mods-table rx-surface">
      ...
      <div className="mods-rows" onClick={onClick} onKeyDown={onKeyDown}>
        {rows.map(r => <ModFamily key={r.family.id} row={r} open={expanded.has(r.family.id)} ilvl={ilvl} floor={floor} perTier={total ? 1 / total : null} canTrade={!!onTrade} canStash={!!onStash} />)}
