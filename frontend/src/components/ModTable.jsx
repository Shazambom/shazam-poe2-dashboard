...
function ModTable({ title, affix, rows, total, expanded, onToggle, onTrade, onStash, ilvl, floor, empty, grants, prices }) {
  ...
  const onClick = (e) => {
    ...
    if (stash) { onStash(affix, stash.dataset.stash); return }
  ...
  return (
    <section className="mods-table rx-surface">
      ...
      <div className="mods-rows" onClick={onClick} onKeyDown={onKeyDown}>
        {rows.map(r => <ModFamily key={r.family.id} row={r} open={expanded.has(r.family.id)} ilvl={ilvl} floor={floor} perTier={total ? 1 / total : null} canTrade={!!onTrade} canStash={!!onStash} grants={grants} prices={prices} />)}
