...
function createHistoryConsumer({ manager, worker, prefs, send = null, log = () => {}, now = Date.now }) {
  ...
  async function onItem(item) {
    ...
    if (!p) { log(`history-skip reason=worker origin=${origin}`); return }
    ...
    const base = { id, source: 'ee2', origin, folder: 'ee2-history', cfgLeague: pf.leagueId || null, ts: now(), buildMs: typeof r.buildMs === 'number' ? r.buildMs : now() - t0 }
    if (r.error) {
      if (r.error.stage === 'currency') { log(`history-skip reason=currency origin=${origin}`); return }
      const name = String(item.name || item.baseType || 'Unknown item').slice(0, 60)
      log(`history-degraded stage=${r.error.stage} name="${name}"`)
      emit({ ...base, q: null, degraded: true, stage: r.error.stage, name, item: { name: item.name || '', baseType: item.baseType || '', rarity: item.rarity || '', itemClass: item.itemClass || '' } })
      return
    }
    ...
    emit({ ...base, q: r.q, degraded: false, stage: null, name: r.name, item: r.item })
  }
